...
func TestBanditScanner_Name(t *testing.T) {
	t.Parallel()
	sc := bandit.New(nopCodec{}, "")
	assert.Equal(t, finding.ScannerName("bandit"), sc.Name())
}

func TestBanditScanner_SupportedLanguages(t *testing.T) {
	t.Parallel()
	sc := bandit.New(nopCodec{}, "")
...
func TestBanditScanner_Available_NonExistentBinary(t *testing.T) {
	t.Parallel()
	sc := bandit.New(nopCodec{}, "/tmp/no-binary")
	assert.False(t, sc.Available(context.Background()))
}

func TestBanditScanner_Scan_BinaryNotFound(t *testing.T) {
	t.Parallel()
	sc := bandit.New(nopCodec{}, "/tmp/no-binary")
