...
func TestGosecScanner_Name(t *testing.T) {
	t.Parallel()
	sc := gosec.New(nopCodec{}, "")
	assert.Equal(t, finding.ScannerName("gosec"), sc.Name())
}

func TestGosecScanner_SupportedLanguages(t *testing.T) {
	t.Parallel()
	sc := gosec.New(nopCodec{}, "")
...
func TestGosecScanner_Available_NonExistentBinary(t *testing.T) {
	t.Parallel()
	sc := gosec.New(nopCodec{}, "/tmp/no-binary")
	assert.False(t, sc.Available(context.Background()))
}

func TestGosecScanner_Scan_BinaryNotFound(t *testing.T) {
	t.Parallel()
	sc := gosec.New(nopCodec{}, "/tmp/no-binary")
