...
func TestSpotBugsScanner_Name(t *testing.T) {
	t.Parallel()
	sc := spotbugs.New(nopCodec{}, "")
	assert.Equal(t, finding.ScannerName("spotbugs"), sc.Name())
}

func TestSpotBugsScanner_SupportedLanguages(t *testing.T) {
	t.Parallel()
	sc := spotbugs.New(nopCodec{}, "")
...
func TestSpotBugsScanner_Available_NonExistentBinary(t *testing.T) {
	t.Parallel()
	sc := spotbugs.New(nopCodec{}, "/tmp/cortex-no-such-binary")
	assert.False(t, sc.Available(context.Background()))
}

func TestSpotBugsScanner_Scan_BinaryNotFound(t *testing.T) {
	t.Parallel()
	sc := spotbugs.New(nopCodec{}, "/tmp/cortex-no-such-binary")
